...
from datasets import Dataset, DatasetDict, Features, Value, Sequence, Array2D, Array3D, load_dataset
...
output_dir = './output'  # Directory to save the model and checkpoints
...
def augment_and_transform_batch(
    ...
    for image_id, image_path, objects in zip(examples["image_id"], examples["image"], examples["objects"]):
        image = Image.open(image_path).convert("RGB")
        image = np.array(image)

        # 提取对象的类别和边界框
        bboxes = [bbox for bbox_list in objects["bbox"] for bbox in bbox_list]  # 展平嵌套的bbox列表
        categories = objects["category_id"]
        areas = objects["area"]
        ...
        formatted_annotations = format_image_annotations_as_coco(
            image_id, categories, areas, output["bboxes"]
        )
        annotations.append(formatted_annotations)

    # 应用图像处理器转换：调整大小、重新缩放、归一化
...
def evaluation_loop(
    ...
    # Record mAP to TensorBoard
    writer.add_scalar('mAP', metrics.get('map', 0.0), epoch)
...
def parse_args():
    ...
    parser.add_argument("--annotation_file", type=str, required=True, help="Path to the COCO annotations file.")
...
def main():
    ...
    feature_extractor = AutoImageProcessor.from_pretrained("./detr-resnet-50")
    # Load local COCO dataset
    dataset = load_dataset("./custom_coco.py", name="custom_coco", trust_remote_code=args.trust_remote_code)
    ...
    # resnet_model.load_state_dict(torch.load("resnet50_weights.pth"))
    ...
    categories = [
        {"id": 1, "name": "Corona"},
        {"id": 2, "name": "Floating Potential"},
        {"id": 3, "name": "Free Particle"},
    ...
    image_processor = AutoImageProcessor.from_pretrained(
        args.model_name_or_path,
        do_resize=True,
        size={"height": args.image_square_size, "width": args.image_square_size},
        do_pad=True,
        ...
        [
            A.Compose(
                [
                    ...
                    A.ShiftScaleRotate(shift_limit=0.3, scale_limit=0, rotate_limit=0, p=0.5),
                ],
                p=0.2,
            ),
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        metrics = evaluation_loop(model, image_processor, accelerator, valid_dataloader, id2label, writer, epoch)
    ...
    metrics = evaluation_loop(model, image_processor, accelerator, test_dataloader, id2label, writer, epoch)
    metrics = {f"test_{k}": v for k, v in metrics.items()}
    for metric_name, metric_value in metrics.items():
        writer.add_scalar(f'Test_Metrics/{metric_name}', metric_value, epoch)
